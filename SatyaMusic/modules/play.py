...
from SatyaMusic.config import ARQ_API_KEY
from SatyaMusic.config import BOT_NAME as bn
from SatyaMusic.config import DURATION_LIMIT
from SatyaMusic.config import UPDATES_CHANNEL as updateschannel
from SatyaMusic.config import que
from SatyaMusic.function.admins import admins as a
from SatyaMusic.helpers.admins import get_administrators
from SatyaMusic.helpers.channelmusic import get_chat_id
from SatyaMusic.helpers.errors import DurationLimitError
from SatyaMusic.helpers.decorators import errors
from SatyaMusic.helpers.decorators import authorized_users_only
from SatyaMusic.helpers.filters import command, other_filters
from SatyaMusic.helpers.gets import get_file_name
from SatyaMusic.services.callsmusic import callsmusic, queues
from SatyaMusic.services.callsmusic.callsmusic import client as USER
from SatyaMusic.services.converter.converter import convert
from SatyaMusic.services.downloaders import youtube
...
def cb_admin_check(func: Callable) -> Callable:
    async def decorator(client, cb):
        ...
        if cb.from_user.id in admemes:
            ...
            await cb.answer("Who are you..?", show_alert=True)
            return
...
@Client.on_message(
    filters.command("mp") & ~filters.edited & ~filters.bot & ~filters.private
)
@authorized_users_only
async def hfmm(_, message):
    ...
    if len(message.command) != 2:
        await message.reply_text(
            "I only recognize `/mp on` and /mp `off only`"
        )
        return
...
async def play(_, message: Message):
    ...
    try:
        ...
        await lel.edit(
            f"<i> {user.first_name} Userbot not in this chat, Ask admin to send /play command for first time or add @a_L_Satya manually</i>"
        )
        return
    text_links=None
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph//file/6868565f8edc3e2221963.jpg"
...
async def deezer(client: Client, message_: Message):
    ...
    try:
        ...
        UserDict.first_name = "DaisyMusic"
    usar = user
    wew = usar.id
    try:
        # chatdetails = await USER.get_chat(chid)
    ...
    try:
        ...
        thumbnail = "https://telegra.ph//file/6868565f8edc3e2221963.jpg"
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your group due to heavy requests for userbot! Make sure user is not banned in group."
                        "\n\nOr manually add @Satyamusic to your Group and try again</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
