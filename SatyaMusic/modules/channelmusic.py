...
from SatyaMusic.modules.play import generate_cover
from SatyaMusic.modules.play import arq
from SatyaMusic.modules.play import cb_admin_check
from SatyaMusic.modules.play import transcode
from SatyaMusic.modules.play import convert_seconds
from SatyaMusic.modules.play import time_to_seconds
from SatyaMusic.modules.play import changeImageSize
...
async def playlist(client, message):
    ...
    queue = que.get(lol)
    if not queue:
        await message.reply_text("Player is not avaliable")
    temp = []
    for t in queue:
        temp.append(t)
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        queue = que.get(lol)
        if not queue:
            await cb.message.edit("Player is na")
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
